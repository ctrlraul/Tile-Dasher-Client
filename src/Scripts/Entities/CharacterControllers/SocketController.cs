...
public partial class SocketController : Node2D
{
	private static readonly Logger Logger = new("SocketController");
	...
	/** The character control update includes the inputs and velocity,
	 * it sets the velocity and then the input is handled afterward,
	 * which results in a jump with 2x force. This is used to prevent that. */
	...
	{
		base._EnterTree();
		Socket.GotRaceCharacterUpdate += OnRaceCharacterUpdate;
	}
	...
	private void OnRaceCharacterUpdate(RaceCharacterUpdate update)
	{
		if (update.id != character.PlayerId)
			return;

		JustJumped = VerticalInput >= 0 && update.VerticalInput < 0;
		
		character.Position = update.Position;
		character.Velocity = update.Velocity;
		HorizontalInput = update.HorizontalInput;
		VerticalInput = update.VerticalInput;
	}
}
